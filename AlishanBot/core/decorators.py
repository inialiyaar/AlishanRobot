...
def add_command(*command_names):
    ...
    commands_pattern = "|".join(
        rf"{cmd}(?:@{BOT_USERNAME})?" for cmd in command_names
    )
    pattern = rf"(?i)^/({commands_pattern})(?:\s+(.*))?$"
...
def callback_query(data):
    def decorator(func):
        Alishan.add_event_handler(func, events.CallbackQuery(data=data.encode() if isinstance(data, str) else data))
        return func
    return decorator    